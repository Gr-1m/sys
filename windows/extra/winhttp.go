...
var winhttp = syscall.NewLazyDLL(sysdll.Add("winhttp.dll"))
...
func WinHttpOpen(userAgent string, accessType int, proxy string, proxyBypass string, flags uint32) (syscall.Handle, error) {
	log.Println("entering into WinHttpOpen function", "user-agent", userAgent, "accessType", accessType, "proxy", proxy, "proxyBypass", proxyBypass, "flags", flags)

	// Convert useragent to a wide string
	pszAgentW, err := syscall.UTF16PtrFromString(userAgent)
	if err != nil {
		log.Println("there was an error converting userAgent to a UTF16 pointer", "userAgent", userAgent, "error", err)
	...
	if accessType != WINHTTP_ACCESS_TYPE_NAMED_PROXY {
		...
		pszProxyW = &p
		log.Println("the access type was NOT set to WINHTTP_ACCESS_TYPE_NAMED_PROXY forcing proxy to be set to WINHTTP_NO_PROXY_NAME", "access type", accessType, "proxy", proxy)

		b := uint16(WINHTTP_NO_PROXY_BYPASS)
		pszProxyBypassW = &b
		log.Println("the access type was NOT set to WINHTTP_ACCESS_TYPE_NAMED_PROXY forcing proxyBypass to be set to WINHTTP_NO_PROXY_BYPASS", "access type", accessType, "proxyBypass", proxyBypass)
	} else {
		pszProxyW, err = syscall.UTF16PtrFromString(proxy)
		if err != nil {
			log.Println("there was an error converting proxy to a UTF16 pointer", "proxy", proxy, "error", err)
			return 0, fmt.Errorf("winhttp WinHttpOpen(): there was an error converting the proxy value '%s' to a UTF16 pointer: %s", proxy, err)
		}

		if proxyBypass != "" {
			pszProxyBypassW, err = syscall.UTF16PtrFromString(proxyBypass)
			if err != nil {
				log.Println("there was an error converting proxyBypass to a UTF16 pointer", "proxyBypass", proxyBypass, "error", err)
	...
	if !errors.Is(err, ERROR_SUCCESS) {
		log.Println("there was an error calling winhttp!WinHttpOpen", "error", err)
...
func WinHttpConnect(hSession syscall.Handle, serverName string, serverPort uint32) (syscall.Handle, error) {
	log.Println("entering into WinHttpConnect function", "session", hSession, "serverName", serverName, "serverPort", serverPort)

	// Convert server name to a LPCWSTR (uint16 pointer)
	pswzServerName, err := syscall.UTF16PtrFromString(serverName)
	if err != nil {
		log.Println("there was an error converting serverName to a UTF16 pointer", "serverName", serverName, "error", err)
	...
	if !errors.Is(err, ERROR_SUCCESS) {
		log.Println("there was an error calling winhttp!WinHttpConnect", "error", err)
...
func WinHttpOpenRequest(hConnect syscall.Handle, method string, path string, version string, referrer string, accessTypes []string, flags uint32) (syscall.Handle, error) {
	log.Println("entering into WinHttpOpenRequest function", "hConnect", hConnect, "method", method, "path", path, "version", version, "referrer", referrer, "accessTypes", accessTypes, "flags", flags)

	// Convert HTTP method to LPCWSTR
	pwszVerb, err := syscall.UTF16PtrFromString(strings.ToUpper(method))
	if err != nil {
		log.Println("there was an error converting method to a UTF16 pointer", "method", method, "error", err)
	...
	pwszObjectName, err := syscall.UTF16PtrFromString(path)
	if err != nil {
		log.Println("there was an error converting path to a UTF16 pointer", "path", path, "error", err)
	...
	if version == "" {
		...
		pwszVersion, err = syscall.UTF16PtrFromString(version)
		if err != nil {
			log.Println("there was an error converting version to a UTF16 pointer", "version", version, "error", err)
	...
	// Convert the version to LPCWSTR
	pwszReferrer, err := syscall.UTF16PtrFromString(referrer)
	if err != nil {
		log.Println("there was an error converting referrer to a UTF16 pointer", "referrer", referrer, "error", err)
	...
	if len(accessTypes) > 0 {
		for i, acceptType := range accessTypes {
			...
			pwszAcceptType, err = syscall.UTF16PtrFromString(acceptType)
			if err != nil {
				log.Println("there was an error converting acceptType to a UTF16 pointer", "acceptType", acceptType, "error", err)
	...
	if !errors.Is(err, ERROR_SUCCESS) {
		log.Println("there was an error calling winhttp!WinHttpOpenRequest", "error", err)
...
func WinHttpSendRequest(hRequest syscall.Handle, headers string, headersLength uint32, optionalData uintptr, optionalDataLen uint32, totalLen uint32, context uintptr) error {
	log.Println("entering into WinHttpSendRequest function", "hRequest", hRequest, "headers", headers, "optionalData", optionalData, "optionalDataLen", optionalDataLen, "totalLen", totalLen, "context", context)

	// Convert headers to LPCWSTR
	lpszHeaders, err := syscall.UTF16PtrFromString(headers)
	if err != nil {
		log.Println("there was an error converting headers to a UTF16 pointer", "headers", headers, "error", err)
	...
	if !errors.Is(err, ERROR_SUCCESS) {
		log.Println("there was an error calling winhttp!WinHttpSendRequest", "error", err)
...
func WinHttpReceiveResponse(hRequest syscall.Handle) error {
	log.Println("entering into WinHttpReceiveResponse function", "hRequest", hRequest)
	...
	if !errors.Is(err, ERROR_SUCCESS) {
		log.Println("there was an error calling winhttp!WinHttpReceiveResponse", "error", err)
...
func WinHttpReadData(hRequest syscall.Handle, size uint32) ([]byte, error) {
	log.Println("entering into WinHttpReadData function", "hRequest", hRequest, "size", size)
	...
	if !errors.Is(err, ERROR_SUCCESS) {
		log.Println("there was an error calling winhttp!WinHttpReadData", "error", err)
...
func WinHttpQueryDataAvailable(hRequest syscall.Handle) (uint32, error) {
	log.Println("entering into WinHttpQueryDataAvailable function", "hRequest", hRequest)
	...
	if !errors.Is(err, ERROR_SUCCESS) {
		log.Println("there was an error calling winhttp!WinHttpQueryDataAvailable", "error", err)
...
func WinHttpQueryHeaders(hRequest syscall.Handle, infoLevel uint32, header string, index uint32) ([]byte, error) {
	log.Println("entering into WinHttpQueryHeaders function", "hRequest", hRequest, "info level", infoLevel, "header", header, "index", index)
	...
	var err error
	if header != "" {
		pwszName, err = syscall.UTF16PtrFromString(header)
		if err != nil {
			log.Println("there was an error converting the header string to a UTF16 pointer", "header", header, "error", err)
	...
	if !errors.Is(err, syscall.ERROR_INSUFFICIENT_BUFFER) {
		log.Println("there was an error calling winhttp!WinHttpQueryHeaders with WINHTTP_NO_OUTPUT_BUFFER to determine the data size", "error", err)
	...
	if !errors.Is(err, ERROR_SUCCESS) {
		log.Println("there was an error calling winHttpQueryHeaders to receive the data", "error", err)
...
func WinHttpCloseHandle(hInternet syscall.Handle) {
	log.Println("entering into WinHttpCloseHandle function", "hInternet", hInternet)
	...
	if !errors.Is(err, ERROR_SUCCESS) {
		log.Println("there was an error calling winhttp!WinHttpCloseHandle", "error", err)
		return
	}
	// Returns TRUE if the handle is successfully closed, otherwise FALSE
	if r != 1 {
		log.Println("winhttp!WinHttpCloseHandle returned something other than TRUE", "return", r)
	}
}
...
func WinHttpSetOption(hInternet syscall.Handle, option uint32, buffer []byte) error {
	log.Println("entering into WinHttpSetOption function", "hInternet", hInternet, "option", option, "buffer", fmt.Sprintf("(%d) 0x%X", len(buffer), buffer))
	dwOption := option
	lpBuffer := buffer
	...
	if !errors.Is(err, ERROR_SUCCESS) {
		log.Println("there was an error calling winhttp!WinHttpSetOption", "option", option, "buffer", fmt.Sprintf("0x%X", buffer), "error", err)
...
func WinHttpAddRequestHeaders(hRequest syscall.Handle, headers string, modifiers uint32) error {
	log.Println("entering into WinHttpAddRequestHeaders function", "hRequest", hRequest, "headers", headers, "modifiers", fmt.Sprintf("%08b", modifiers))
	...
	if headers != "" {
		lpszHeaders, err = syscall.UTF16PtrFromString(headers)
		if err != nil {
			log.Println("there was an error converting the header string to a UTF16 pointer", "headers", headers, "error", err)
	...
	if !errors.Is(err, ERROR_SUCCESS) {
		log.Println("there was an error calling winhttp!WinHttpAddRequestHeaders", "headers", headers, "modifiers", fmt.Sprintf("%08b", modifiers), "error", err)
